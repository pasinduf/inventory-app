...
const OrderSummary = () => {
  ...
  const [data, setData] = useState<OrderSummaryListResponse | null>(null);
  ...
  const changeDateFilter = (filter: string) => {
    ...
    switch (filter) {
      ...
      case "week": {
        ...
        fetchOrderSummary(format(start, "yyyy-MM-dd"), today);
        break;
      }
      case "month": {
  ...
  const paginatedData = orderRecords?.slice(startIndex, startIndex + itemsPerPage);
          ...
          <div className="flex items-center gap-4">
            ...
            {dateFilter === "custom" && (
              <div className="flex flex-[4]">
                ...
                <div className="ml-2">
                  <Button variant="outline" className="w-full" onClick={() => fetchOrderSummary(format(fromDate, "yyyy-MM-dd"), format(toDate, "yyyy-MM-dd"))}>
                    Search
                  </Button>
                </div>
              </div>
